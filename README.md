...
1. [Overview of iVSR](#1-overview-of-ivsr)
    ...
    - [Capabilities of iVSR](#14-capabilities-of-ivsr)
        ...
        - [SPAN — Single-Image Super Resolution](#145-span--single-image-super-resolution)
...
- #### iv. RIFE — Frame Interpolation
  ...
  Export the model with `export_rife_openvino.py` before use. Input must be `rgb24`; the model input tensor is `[1, 6, H, W]` (two RGB frames concatenated) normalised to [0, 1]. Input width and height must be multiples of 128.<br><br>
  ```plaintext
...
### 1.4.5. SPAN — Single-Image Super Resolution
`SPAN` (Swift Parameter-free Attention Network) is a fast, high-quality single-frame super-resolution model. It upscales each video frame independently (no temporal context) with very low latency.

Export your SPAN checkpoint to OpenVINO IR using `span/export_span_openvino.py`, then run it through FFmpeg using the JSON config system (`model_type=-1`). Separate models must be exported for each scale factor (2× or 4×).

```plaintext
Input shape:  [1, 3, H, W]  float32 [0, 1]  (rgb24 ÷ 255)
Output shape: [1, 3, H×scale, W×scale]  float32 [0, 1]
```
...
### Run SPAN super-resolution

Step 1 — Export the SPAN checkpoint to OpenVINO IR:
```bash
cd span/
python3 export_span_openvino.py --weights spanx4_ch48.pth --scale 4 --channels 48
# → spanx4_ch48_ir/spanx4_ch48.xml + .bin
```

Step 2 — Run with FFmpeg (the JSON config is auto-generated by the Streamlit app, or use the template):
```bash
...
```
For 2× upscaling, export with `--scale 2` and update `"name"` to `"SPAN-x2"`; all other JSON fields remain identical.
